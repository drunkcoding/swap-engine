...
unset key
plot '%s.csv' using 2:xtic(1) title 'DS(SSD)' ls 1, '' using 3 title 'DS(CPU)' ls 2, '' using 4 title 'Archer(CPU+SSD)' ls 4
"""
...
for model in models:
    ...
    with open("plots/e2e_lat_multi.gnuplot", "w") as f:
        f.write(GNU_SCRIPT % tuple([f"plots/{name}"] * 2))

    os.system("gnuplot plots/e2e_lat_multi.gnuplot")
